...
Lesson learned from IN\_DIR bug — each pass should write to its own folder:
...
Yes I think that two-stage pass makes sense
...
Prime Video link — why it's high priority: A reader who just decided the film is better than the book is in the perfect mindset to stream it immediately. That's a warm lead with purchase intent. Amazon's streaming affiliate rates are separate from book rates and can be significant. Every "Movie Wins" and "Too Close to Call" page should have a "Watch on Prime →" CTA alongside "Buy the Book →". It's a second revenue stream on the same page.



Two-stage greenfield — why it's the right approach:



Stage 1 — Factual extraction (cheap model, fast):
...
You verify this is accurate before spending Sonnet tokens on the editorial content.



Stage 2 — Editorial generation (Sonnet, the real work):
...
Stage 1 catches hallucinated facts (wrong director, wrong year) before they contaminate the editorial

Stage 1 is Haiku-cheap — maybe $0.001 per page

Stage 2 only runs on verified data
